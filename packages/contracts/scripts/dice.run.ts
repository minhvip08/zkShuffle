...
class GameService {
  ...
  public async initializeContracts() {
    if (!this.shuffleManager || !this.shuffleGame) {
      ...
      await writeFile(
        path.join(this.zkshuffleDir, "shuffle_manager_contract.txt"),
  ...
  public async initShuffle(clientId: string, numberOfPlayers: number) {
    try {
      if (this.shuffleGame && this.players) {
        await (
          await this.shuffleGame.connect(this.players[0]).newGame(clientId, numberOfPlayers)
        ).wait();
        const gameId = (
          await this.shuffleGame.connect(this.players[0]).getShuffleGameId(clientId)
        ).toNumber();

        console.log("Player ", this.players[0].address, "Create Game ", clientId);
  ...
  async playerRun(
    ...
    console.log("Player ", owner.address, "Join Game ", gameId, " asigned playerId ", playerIdx);
    callback();
...
async function handleDiceRoll(message: RequestMessage) {
  ...
  try {
    if (gameId !== null) {
      ...
      const playerRun = gameService.playerRun(gameId, () => {
        clientWs?.send(
          JSON.stringify({
      ...
      if (result !== null) {
        console.log(`Dice rolled result for Client result !== null ${message.clientId}: ${result}`);
        clientWs?.send(
          JSON.stringify({
        ...
        console.log(`Failed to roll dice for Client ${message.clientId}`);
        clientWs?.send(
          JSON.stringify({
            event: "RESULT_DICE_ROLLED",
    ...
    const clientWs = clients.get(message.clientId);
    console.error(`Error rolling dice for Client ${message.clientId}:`, error);
    clientWs?.send(
      JSON.stringify({
        event: "RESULT_DICE_ROLLED",
        result: Math.random() * 6,
